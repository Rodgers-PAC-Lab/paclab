...
def load_persons_metadata_sheet(person, drop_long_columns=True):
    """Load data from `person`'s metadata sheet
    ...
    if person == 'abigail':
        ...
        url = (
            'https://docs.google.com/spreadsheets/d/' # google prefix
            '1VyTdYuG2pioR1A1-bCi4Eb0nXMG6V9ByBRpn6OBcvjU/' # doc ID
            'export?format=xlsx' # export command
            )    
    ...
    df['session_date'] = pandas.to_datetime(df['session_date']).dt.date
...
def abigail_sheet(drop_long_columns=True):
    ...
    print(
        "warning: replace abigail_sheet() "
        "with load_persons_metadata_sheet('abigail')")
    return load_persons_metadata_sheet(
        person='abigail',
        drop_long_columns=drop_long_columns)
    
def cedric_ad_sheet(drop_long_columns=True):
    """See docstring in load_persons_metadata_sheet"""
    print(
        "warning: replace cedric_ad_sheet() "
        "with load_persons_metadata_sheet('cedric')")
    return load_persons_metadata_sheet(
        person='cedric',
        drop_long_columns=drop_long_columns)
